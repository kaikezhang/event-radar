...
你是一个 **每天交易美股的 day trader**，每月付 $29 使用 Event Radar。你关心的是：
- **这条信息能帮我做交易决策吗？** 如果不能，它为什么出现在这里？
- **我看到的内容有价值吗？** "AAPL entered StockTwits trending" 连续出现 4 次 = 垃圾
- **这个产品值 $29/月吗？** 如果你觉得不值，说出为什么
- **哪里让你觉得烦/困惑/无聊？** 这比任何 bug 都重要

**不要只检查"功能是否正常工作"。要检查"功能展示的内容是否对用户有价值"。**

具体来说：
- 看到一个事件列表 → 问自己："如果我是 trader，这里面哪些信息我会在意？哪些是浪费我时间的噪音？"
- 看到一个数字/指标 → 问自己："这个数字帮我做了什么决策？如果答案是'没有'，它不应该在这里"
- 看到重复内容 → 这是 bug，不是 feature
- 看到空页面/无数据 → 问自己："用户看到这个会怎么想？会觉得产品坏了吗？"
- 看到任何让你想关闭浏览器的东西 → 记下来，这就是流失点

**你的 QA 报告应该同时包含：**
1. 功能性 bug（按钮坏了、页面报错）
2. **内容质量问题**（垃圾数据、重复信息、噪音太多）
3. **产品体验问题**（作为用户，哪里让你失望了？）
