...
describe('EventDeduplicator', () => {
  ...
  it('should detect duplicate via DB lookup when sourceEventId matches', async () => {
    const existingDbRow = {
      id: '00000000-0000-0000-0000-000000000001',
      source: 'sec-edgar',
      sourceEventId: 'filing-abc-123',
      title: 'Old event',
      summary: null,
      rawPayload: null,
      metadata: null,
      ...
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            limit: vi.fn().mockResolvedValue([existingDbRow]),
          }),
        }),
      }),
    };

    const dedupWithDb = new EventDeduplicator({
      windowMs: 30 * 60 * 1000,
      db: mockDb as any,
    });

    const event = makeEvent({ id: 'filing-abc-123' });
    const result = await dedupWithDb.check(event);

    expect(result.isDuplicate).toBe(true);
    expect(result.matchType).toBe('db-lookup');
    expect(result.matchConfidence).toBe(1.0);
    expect(result.originalEventId).toBe('00000000-0000-0000-0000-000000000001');
  });

  it('should fall through to in-memory check when DB finds no match', async () => {
    const mockDb = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
