...
const TEST_API_KEY = 'historical-api-key';
...
describe('historical routes', () => {
  ...
  beforeEach(async () => {
    await cleanTestDb(sharedDb);
    await cleanHistoricalTables(sharedDb);
    const { strongestEventId } = await seedHistoricalFixture(sharedDb);
    ...
    expect(strongestEventId).toBeTruthy();
  });
  ...
  it('returns similarity matches from query parameters', async () => {
    const response = await ctx.server.inject({
      method: 'GET',
      url: '/api/historical/similar?eventType=earnings&eventSubtype=beat&sector=Technology&severity=high&vixLevel=20&marketRegime=bull&return30d=0.08&marketCapTier=mega&epsSurprisePct=10&consecutiveBeats=4&limit=2&minScore=8',
      headers: {
  ...
  it('lists historical events with contexts and returns', async () => {
    const response = await ctx.server.inject({
      method: 'GET',
      url: '/api/historical/events?eventType=earnings&severity=high,critical&limit=2&offset=0',
      headers: {
        'x-api-key': TEST_API_KEY,
      },
    });
    ...
    expect(response.json()).toMatchObject({
      data: [
        expect.objectContaining({
          ticker: 'AAPL',
          eventType: 'earnings',
          stockContext: expect.objectContaining({
            marketCapTier: 'mega',
          }),
  ...
  it('returns a single historical event with sources and metrics', async () => {
    const response = await ctx.server.inject({
      method: 'GET',
      url: '/api/historical/events/10000000-0000-0000-0000-000000000001',
      headers: {
        'x-api-key': TEST_API_KEY,
      },
    });
  ...
  it('returns aggregated historical patterns', async () => {
    const response = await ctx.server.inject({
      method: 'GET',
      url: '/api/historical/patterns',
      headers: {
        'x-api-key': TEST_API_KEY,
      },
    });
  ...
  it('returns database summary stats for historical coverage', async () => {
    const response = await ctx.server.inject({
      method: 'GET',
      url: '/api/historical/stats',
      headers: {
        'x-api-key': TEST_API_KEY,
      },
    });
    ...
    expect(response.json()).toMatchObject({
      totalEvents: 6,
      eventsWithReturns: 6,
      eventsWithMarketContext: 6,
      eventsWithStockContext: 6,
      uniqueTickers: 5,
      coverage: {
        scans: 2,
        completedScans: 1,
      ...
      timeRange: {
        earliest: '2023-01-27T21:00:00.000Z',
        latest: '2026-01-15T21:00:00.000Z',
      },
      eventTypes: [
        expect.objectContaining({
          eventType: 'earnings',
      ...
      topTickers: expect.arrayContaining([
        expect.objectContaining({
          ticker: 'PFE',
          count: 2,
        }),
