...
beforeEach(() => {
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === '/api/price/batch') {
      const requested = (url.searchParams.get('tickers') ?? '').split(',').filter(Boolean);
      return jsonResponse(Object.fromEntries(requested.map((ticker) => [
        ticker.toUpperCase(),
        {
          price: ticker.toUpperCase() === 'TSLA' ? 212.75 : 178.5,
          change: ticker.toUpperCase() === 'TSLA' ? -3.4 : 2.3,
          changePercent: ticker.toUpperCase() === 'TSLA' ? -3.4 : 2.3,
        },
      ])));
    }
