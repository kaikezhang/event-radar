      ...
      <div className="lg:flex lg:gap-6">
        ...
        <div className="min-w-0 space-y-4 lg:flex-[2]">
          ...
          <div id="zone-verdict" ref={verdictRef} className="scroll-mt-28">
            ...
            {/* Source Details — source-specific structured data */}
            {data.sourceMetadata && Object.keys(data.sourceMetadata).length > 0 && (
              <SourceDetailsSection source={data.sourceKey ?? data.source} metadata={data.sourceMetadata} />
            )}
          </div>
...
// ── Source Details Section (EventDetail) ──────────────────────────────────────

function SourceDetailsSection({
  source,
  metadata,
}: {
  source: string;
...
function sourceDetailTitle(source: string): string {
  switch (source) {
    case 'breaking-news': return 'News Source';
    case 'sec-edgar': return 'SEC Filing Details';
    case 'trading-halt': return 'Halt Details';
    case 'econ-calendar': return 'Economic Indicator';
    case 'stocktwits': return 'StockTwits Activity';
    case 'reddit': return 'Reddit Activity';
...
function renderSourceDetails(
  ...
  switch (source) {
    case 'breaking-news': {
      const feed = m.sourceFeed as string | undefined;
      ...
      return (
        <div className="space-y-2 text-[15px] leading-7 text-text-secondary">
          {feed && <p>📰 Source: <span className="font-medium text-text-primary">{feed}</span></p>}
          {url && (
            <p>
              🔗{' '}
    ...
    case 'sec-edgar': {
      const formType = m.formType as string | undefined;
      const company = m.companyName as string | undefined;
      const items = m.itemDescriptions as string[] | undefined;
      const link = m.filingLink as string | undefined;
      if (!formType && !company && !items?.length) return null;
      return (
        <div className="space-y-2 text-[15px] leading-7 text-text-secondary">
          {formType && <p>📋 Form Type: <span className="font-semibold text-text-primary">{formType}</span></p>}
    ...
    case 'trading-halt': {
      const isResume = m.isResume as boolean | undefined;
      const code = m.haltReasonCode as string | undefined;
      const desc = m.haltReasonDescription as string | undefined;
      const haltTime = m.haltTime as string | undefined;
      const resumeTime = m.resumeTime as string | undefined;
    ...
    case 'econ-calendar': {
      const name = m.indicatorName as string | undefined;
      const scheduled = m.scheduledTime as string | undefined;
      const freq = m.frequency as string | undefined;
      const tags = m.tags as string[] | undefined;
      if (!name && !freq) return null;
      return (
        <div className="space-y-2 text-[15px] leading-7 text-text-secondary">
          {name && <p>📋 Indicator: <span className="font-medium text-text-primary">{name}</span></p>}
          {scheduled && <p>⏱ Scheduled: {formatTimeShort(scheduled)}</p>}
          {freq && <p>🔄 Frequency: <span className="capitalize">{freq}</span></p>}
          {tags && tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {tags.map((tag, i) => (
                <span key={i} className="rounded-full bg-bg-elevated px-2.5 py-0.5 text-[12px] text-text-tertiary">
                  {tag}
                </span>
              ))}
            </div>
    ...
    case 'stocktwits': {
      const current = m.currentVolume as number | undefined;
      const previous = m.previousVolume as number | undefined;
      const ratio = m.ratio as number | undefined;
      if (current == null && ratio == null) return null;
      return (
        <div className="space-y-2 text-[15px] leading-7 text-text-secondary">
          {current != null && <p>📊 Message volume: <span className="font-medium text-text-primary">{current}</span>{previous != null ? ` (prev: ${previous})` : ''}</p>}
          {ratio != null && <p>📈 Sentiment ratio: <span className="font-medium text-text-primary">{ratio.toFixed(2)}</span></p>}
          {current != null && previous != null && previous > 0 && (
            <p>🔥 Volume spike: <span className="font-medium text-text-primary">{(current / previous).toFixed(1)}x</span> normal</p>
          )}
        </div>
      );
    }
    ...
    case 'reddit': {
      ...
      const highEngagement = m.highEngagement as boolean | undefined;
      if (upvotes == null && comments == null) return null;
      return (
        <div className="space-y-2 text-[15px] leading-7 text-text-secondary">
          {upvotes != null && <p>⬆️ Upvotes: <span className="font-medium text-text-primary">{upvotes.toLocaleString()}</span></p>}
          {comments != null && <p>💬 Comments: <span className="font-medium text-text-primary">{comments.toLocaleString()}</span></p>}
